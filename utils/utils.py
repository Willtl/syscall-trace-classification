...
class ExperimentTracker:
    def __init__(self, model, optimizer, scheduler, label_encoder, args, base_dir='experiments', metric='f1_score', save_interval=50):
        self.model = model
        ...
        self.metrics_tracker = MetricsTracker(label_encoder, confusion_interval=save_interval, run_dir=self.run_dir)
        self.save_arguments()
    ...
    def update_and_save(self, epoch, train_loss, train_preds, train_labels, test_preds, test_labels, current_lr):
        self.metrics_tracker.update(train_loss, train_preds, train_labels, test_preds, test_labels, current_lr)
        ...
        if self.metrics_tracker.metrics[self.metric][-1] == self.metrics_tracker.best_metrics[self.metric]:
...
class MetricsTracker:
    def __init__(self, label_encoder, confusion_interval=10, run_dir=None):
        self.metrics = {'train_loss': [], 'train_acc': [], 'test_acc': [], 'precision': [], 'recall': [], 'f1_score': [], 'learning_rate': []}
        self.best_metrics = {'train_loss': float('inf'), 'train_acc': 0, 'test_acc': 0, 'precision': 0, 'recall': 0, 'f1_score': 0}
        self.label_encoder = label_encoder
        self.confusion_interval = confusion_interval
        self.epoch = 0
        self.run_dir = run_dir

    def update(self, train_loss, train_preds, train_labels, test_preds, test_labels, current_lr):
        self.epoch += 1
    ...
    def update_best_metrics(self, train_loss, train_acc, test_acc, precision, recall, f1):
    ...
    def plot_confusion_matrix(self, preds, labels):
        ...
        cm = confusion_matrix(string_labels, string_preds, labels=self.label_encoder.classes_)
...
def build_custom_tokenizer(sequences, mode='numeric'):
    ...
    if mode == 'numeric':
        ...
        str_sequences = [" ".join(sublist) if isinstance(sublist, list) else sublist for sublist in sequences]
...
def pool_logits(logits, labels, chunk_indices):
    ...
    for chunk_idx in pooled_logits_dict:
        pooled_logits.append(torch.mean(torch.stack(pooled_logits_dict[chunk_idx]), dim=0))
